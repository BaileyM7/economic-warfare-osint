...
shown to everyone — its "Launch app" CTAs route to `/risk-feed` when a token exists,
`/login` otherwise (`launchTarget()` in `pages/landing/hooks.ts`). Its demo clips live
in `public/videos/` and are recorded via the repo-root `demo-videos/` pipeline.
