...
async function runAction(action) {
  ...
  // Local extension (Emissary): visible typing + key press for demo inputs.
  if (fn === 'type') {
    await page.click(sel, { timeout: 10_000 }).catch(err => console.warn(`  [warn] type(${sel}) click failed:`, err.message));
    await page.type(sel, String(action.value ?? ''), { delay: opts?.delay ?? 40 })
      .catch(err => console.warn(`  [warn] type(${sel}) failed:`, err.message));
    return null;
  }
  if (fn === 'press') {
    await page.keyboard.press(String(action.value ?? 'Enter'))
      .catch(err => console.warn(`  [warn] press failed:`, err.message));
    return null;
  }
...
async function checkAction(beat, action) {
  const { fn, sel } = action;
  if (fn === 'wait' || fn === 'press') return null;
  if (fn === 'unhl' && !sel) return null;   // bare unhl clears all highlights — no selector to probe
  if (fn === 'navigate') {
    try {
      await page.goto(targetUrl(sel), { waitUntil: 'networkidle', timeout: 20_000 });
