...
def test_officers_collapse_to_one_candidate_per_person():
    officers = [
        Officer(
            ...
            role="director",
            company_name="Renova Group",
            company_jurisdiction="ru",
            company_number="111",
        ),
        Officer(
            name="Viktor Vekselberg",
            role="director",
            end_date=None,
            company_name="Renova US Inc",
            company_jurisdiction="us_de",
            company_number="222",
        ),
...
def test_merge_unions_sources_and_keeps_sanctions_data():
    csl_only = [
        PersonCandidate(
            ...
            sanctioned=True,
            sanction_programs=["UKRAINE-EO13661"],
            country="RU",
        ),
    ]
    oc_only = [
        PersonCandidate(
            name="VIKTOR VEKSELBERG", sources=["opencorporates"], primary_affiliation="Renova Group"
        ),
    ]
...
def test_prune_caps_l2_per_l1_parent():
    ...
    for i in range(50):
        nodes.append(PersonNetworkNode(id=f"p_l2_{i}", label=f"L2_{i}", group="person", depth=2))
...
def test_pep_current_position_is_expected():
    ...
    factors = build_risk_factors(
        {
            "pep_hits": [
                {"name": "X", "positions": ["President"], "parties": [], "is_current": True}
            ],
        }
    )
...
def test_pep_party_only_is_suggested():
    ...
    factors = build_risk_factors(
        {
            "pep_hits": [
                {"name": "X", "positions": [], "parties": ["United Russia"], "is_current": False}
            ],
        }
    )
    pep = next(f for f in factors if f.title == "Political Exposure")
...
def test_merge_unions_alt_names():
    csl_only = [
        PersonCandidate(
            ...
            sources=["opensanctions"],
            sanctioned=True,
            alt_names=["Viktor F. Vekselberg"],
        ),
    ]
    oc_only = [
        PersonCandidate(
            name="VIKTOR VEKSELBERG", sources=["opencorporates"], alt_names=["Vekselberg V."]
        ),
    ]
    merged = _merge_candidates(csl_only, oc_only)
    assert len(merged) == 1
