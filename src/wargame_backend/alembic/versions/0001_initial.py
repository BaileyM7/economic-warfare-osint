...
def upgrade() -> None:
    ...
    op.execute("CREATE TYPE data_source_status AS ENUM ('active', 'degraded', 'disabled')")
    op.execute(
        "CREATE TYPE event_domain AS ENUM "
        "('info', 'diplomatic', 'economic', 'cyber', "
        "'kinetic_limited', 'kinetic_general')"
    )
    op.execute("CREATE TYPE scenario_status AS ENUM ('draft', 'ready', 'archived')")
    op.execute(
        "CREATE TYPE simulation_status AS ENUM "
        "('pending', 'running', 'paused', 'completed', 'aborted', 'error')"
    )
    op.execute("CREATE TYPE memory_type AS ENUM ('observation', 'decision', 'intel', 'doctrine')")
    ...
    op.create_index(
        "ix_countries_profile_gin",
        "countries",
        ["profile"],
        postgresql_using="gin",
    )
    op.create_index(
        "ix_countries_doctrine_gin",
        "countries",
        ["doctrine"],
        postgresql_using="gin",
    )
    ...
    op.create_table(
        ...
        sa.Column("alliance_memberships", postgresql.JSONB, nullable=False, server_default="[]"),
        sa.Column(
            "updated_at", sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "status",
            postgresql.ENUM(
                "active", "degraded", "disabled", name="data_source_status", create_type=False
            ),
            nullable=False,
    ...
    op.create_table(
        ...
        sa.Column(
            "status",
            postgresql.ENUM(
                "draft", "ready", "archived", name="scenario_status", create_type=False
            ),
            nullable=False,
    ...
    op.create_table(
        ...
        sa.Column(
            "parent_event_id",
            postgresql.UUID(as_uuid=True),
            sa.ForeignKey("sim_events.id", ondelete="SET NULL"),
            nullable=True,
        ),
    ...
    op.execute(
        "ALTER TABLE agent_memory ALTER COLUMN embedding TYPE vector(1536) USING embedding::vector(1536)"
    )
