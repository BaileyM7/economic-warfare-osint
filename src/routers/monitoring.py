...
_ENTITY_GEO: dict[str, tuple[float, float]] = {
    ...
    "tsmc": (24.8, 121.0),
    "samsung": (37.4, 127.0),
    "central state bank": (39.9, 32.9),
    "eastern shipping": (1.3, 103.8),
    "oceanic holdings": (1.3, 103.8),
    "pacific maritime": (1.3, 103.8),
    "ganfeng lithium": (27.1, 114.9),
    "tianqi lithium": (30.6, 104.1),
    "catl": (26.7, 119.3),
    "china mobile": (39.9, 116.4),
    "xiaomi": (39.9, 116.4),
    "baidu": (39.9, 116.4),
    "tencent": (22.5, 114.1),
    "nio": (31.2, 121.5),
...
async def monitoring_map_data():
    ...
    try:
        rows = conn.execute(
            "SELECT name, target_entities, status FROM coas WHERE status NOT IN ('assessed')"
        ).fetchall()
    finally:
        conn.close()
...
async def get_monitoring_macro():
    ...
    try:
        ...
        # USD/CNY — use _fetch_info (the only YFinanceClient method that
        # exposes raw .info). The previous `yf.get_info("CNY=X")` call hit a
        # method that doesn't exist on YFinanceClient, raising AttributeError
        # that was silently swallowed by the outer except.
        try:
            info = await asyncio.to_thread(YFinanceClient._fetch_info, "CNY=X")
            if not info.get("_data_unavailable"):
