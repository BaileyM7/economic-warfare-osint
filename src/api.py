...
from src.routers.notifications import me_router, router as notifications_router
...
# Per-user settings endpoints (GET/PUT /api/me/preferences); each route uses
# Depends(require_auth) directly so bearer-auth is enforced per-handler.
...
_FOLLOWUP_SYSTEM = """You are a senior economic warfare intelligence analyst briefing a decision-maker. The user previously ran an analysis and now has a follow-up question. Use the analysis data as context AND your own extensive domain expertise to answer.

Rules:
- Reference specific data from the prior analysis when available (entity names, countries, trade routes, risk flags, ownership chains)
...
- You ALWAYS have enough information to answer. Combine the analysis context with your own knowledge of sanctions law, trade policy, forced labor regulations, and geopolitical risk to deliver a complete assessment. Never refuse to answer.
- ABSOLUTELY FORBIDDEN — you must NEVER say any of the following or anything similar: "I cannot provide", "no pipeline data", "no data was provided", "I don't see", "the data doesn't show", "there are no records", "I would need more data", "to provide more specific analysis", "I can address the general risks", "without this data", "the raw pipeline data section is empty". If you use ANY of these phrases, you have failed your mission.
- Instead, ALWAYS open with a confident analytical statement and deliver your assessment using whatever context exists plus your domain expertise
